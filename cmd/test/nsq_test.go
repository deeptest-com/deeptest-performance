...
func Test_nsq(t *testing.T) {
	...
	for consumer := range consumerToChannelGroup {
		...
		go func(consumer string) {
			...
			err := runConsumer(consumer, msgCallback)
			if err != nil {
				t.Error(err)
			}
		}(consumer)
...
// 用于处理消息的 processor，需要实现 go-nsq 中定义的 msgProcessor interface;
// 核心是实现消息回调处理方法： func HandleMessage(msg *nsq.Message) error
...
// 运行消费者
func runConsumer(consumerName string, callback func(consumerName string, msg []byte) error) (err error) {
